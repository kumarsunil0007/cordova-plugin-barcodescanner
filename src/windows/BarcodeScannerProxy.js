...
function findCamera() {
    ...
    return Devices.DeviceInformation.findAllAsync(
        Devices.DeviceClass.videoCapture
        ...
        var backCameras = cameras.filter(function (camera) {
            return (
                camera.enclosureLocation &&
                camera.enclosureLocation.panel === Devices.Panel.back
            );
        });
...
BarcodeReader.prototype.init = function (capture, width, height) {
    ...
    var formatsList =
        BarcodeReader.scanCallArgs.args.length > 0 &&
        BarcodeReader.scanCallArgs.args[0].formats;
    if (formatsList) {
        var possibleFormats = formatsList.split(",").map((format) => {
...
BarcodeReader.prototype.readCode = function () {
    ...
    function scanBarcodeAsync(
        ...
        var frame = new Windows.Media.VideoFrame(
            Imaging.BitmapPixelFormat.bgra8,
            frameWidth,
            frameHeight
        );
        return mediaCapture
            .getPreviewFrameAsync(frame)
            .then(function (capturedFrame) {
                ...
                var rawBuffer = new Streams.Buffer(
                ...
                return zxingReader.decode(
                    data,
                    frameWidth,
                    frameHeight,
                    ZXing.BitmapFormat.bgra32
                );
            });
    }

    var self = this;
    return scanBarcodeAsync(
        this._capture,
        this._zxingReader,
        this._width,
        this._height
...
module.exports = {
    ...
    scan: function (success, fail, args) {
        ...
        function createPreview() {
            ...
            capturePreviewFrameStyle.href = urlutil.makeAbsolute(
                "/www/css/plugin-barcodeScanner.css"
            );
        ...
        function focus(controller) {
            ...
            if (!controller) {
                try {
                    ...
                    console.log(
                        "Failed to access focus control for current camera: " +
                            err
                    );
                    return result;
                }
            ...
            if (
                ...
                !controller.focusControl.supported
            ) {
                console.log(
                    "Focus control for current camera is not supported"
                );
                return result;
            }
            ...
            // somehow is not accessible there. See https://github.com/kumarsunil0007/cordova-plugin-barcodescanner/issues/288
            try {
                if (
                    controller.focusControl.focusState ===
            ...
            return WinJS.Promise.timeout(INITIAL_FOCUS_DELAY).then(function () {
                try {
                    return controller.focusControl.focusAsync().then(
                        function () {
                            return result;
                        },
                        function (e) {
        ...
        function setupFocus(focusControl) {
            function supportsFocusMode(mode) {
                return focusControl.supportedFocusModes.indexOf(mode)
                    .returnValue;
            }
            ...
            if (supportsFocusMode(FocusMode.continuous)) {
                ...
                console.log(
                    "Device doesn't support continuous focus mode, switching to autofocus mode"
                );
            ...
            function waitForIsPlaying() {
                var isPlaying =
                    !capturePreview.paused &&
                    !capturePreview.ended &&
                    capturePreview.readyState > 2;

                if (!isPlaying) {
                    return WinJS.Promise.timeout(CHECK_PLAYING_TIMEOUT).then(
                        function () {
        ...
        function startPreview() {
            return findCamera()
                .then(function (id) {
                    ...
                    try {
                        ...
                        if (e.number === REGDB_E_CLASSNOTREG) {
                            throw new Error(
                                "Ensure that you have Windows Media Player and Media Feature pack installed."
                            );
                        }
                ...
                .then(function () {
                    ...
                    var deviceProps =
                        controller.getAvailableMediaStreamProperties(
                ...
                .then(function (captureSettings) {
                    ...
                    return setupFocus(
                        captureSettings.capture.videoDeviceController
                            .focusControl
                    )
                        .then(function () {
                            Windows.Graphics.Display.DisplayInformation.getForCurrentView().addEventListener(
                                "orientationchanged",
                            ...
                            function checkCameraStreamState() {
                                if (
                                    ...
                                    return WinJS.Promise.timeout(
                                        CAMERA_STREAM_STATE_CHECK_RETRY_TIMEOUT
                                    ).then(function () {
        ...
        function destroyPreview() {
            ...
            Windows.Graphics.Display.DisplayInformation.getForCurrentView().removeEventListener(
                "orientationchanged",
                updatePreviewForRotation,
                false
            );
         ...
         * See https://github.com/kumarsunil0007/cordova-build/BarcodeScanner#using-the-plugin
         */
        ...
        BarcodeReader.scanPromise.done(
            function () {},
            function (error) {
                // Suppress null result (cancel) on suspending
                if (BarcodeReader.suspended) {
...
function suspend(args) {
    ...
    if (args) {
        args.setPromise(
            BarcodeReader.destroyPreview().then(waitForScanEnd, waitForScanEnd)
        );
    } else {
...
function resume() {
    ...
    module.exports.scan(
        BarcodeReader.scanCallArgs.success,
        BarcodeReader.scanCallArgs.fail,
        BarcodeReader.scanCallArgs.args
    );
}
...
function onVisibilityChanged() {
    if (
        ...
        BarcodeReader.videoPreviewIsVisible &&
        BarcodeReader.videoPreviewIsVisible() &&
        BarcodeReader.destroyPreview
    ) {
        suspend();
